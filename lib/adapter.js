...
var LOG_QUERIES = true //process.env.LOG_QUERIES; //It shows all executed queries
var LOG_DEBUG = true //process.env.LOG_DEBUG;  //It show messages which allow you to follow the code
    ...
    var adapter = {
        ...
        query: function (connectionName, table, query, data, cb) {
            ...
            connectionObject.pool.getConnection(
                    function (err, connection)
                    {
                        ...
                        if (LOG_QUERIES) {
                            console.log('SAILS-ORA-ADAPTER: Executing PLAIN_SQL query: ' + query);
                        }
                        ...
                        connection.execute(query, data, {maxRows:10000, outFormat: oracledb.OBJECT}, function (err, result) {
                            ...
                            // console.log("SAILS-ORA-ADAPTER: The query's result before castClobs is: " + JSON.stringify(result));
                            ...
                            return castClobs(result, function (err, result) {
                                ...
                                if (LOG_QUERIES) {
                                    ...
                                    // console.log("SAILS-ORA-ADAPTER: RESULT OF PLAIN_SQL query is: " + JSON.stringify(result.rows))
                                }
                                doRelease(connection);
        ...
        join: function (connectionName, table, options, cb) {
            ...
            Cursor({
                ...
                $populateBuffers: function populateBuffers(options, next) {
                    ...
                    // console.log("SAILS-ORA-ADAPTER: result of _schema: " + JSON.stringify(_schema));
                    // console.log("SAILS-ORA-ADAPTER: result of _options: " + JSON.stringify(_options));
                    // console.log("SAILS-ORA-ADAPTER: result of instructions: " + JSON.stringify(instructions));
                    ...
                    // console.log("SAILS-ORA-ADAPTER: the provided instructions are: " + JSON.stringify(instructions));

                    // The natively formed query results in SQL queries with huge lengths causing ORA-00972: identifier is too long error. This will be addressed below using some regex string replacement
                    var tableName=Object.keys(instructions.instructions);
                    console.log("SAILS-ORA-ADAPTER: the table name is: " + tableName);
                    var longAlias = instructions.instructions[tableName].instructions[0].alias;
                    var parentKey = instructions.instructions[tableName].instructions[0].parentKey;
                    longAlias = "__" + longAlias;
                    parentKey = parentKey + "___";
                    replaceLongAlias=new RegExp(longAlias,"g");
                    replaceParentKey=new RegExp(parentKey,"g");
                    findQuery=findQuery.replace(replaceLongAlias, "A");
                    findQuery=findQuery.replace(replaceParentKey, "");
        ...
        find: function (connectionName, table, options, cb) {
            if (LOG_DEBUG) {
                ...
                console.log('SAILS-ORA-ADAPTER: the value of options is: ' + JSON.stringify(options));
            }
            // Grab Connection Schema
            var schema = {};
            var connectionObject = connections[connectionName];
            ...
            findQuery = findQuery.replace(" ILIKE ", " LIKE ");
            ...
            connectionObject.pool.getConnection(
                    function (err, connection)
                    {
                        ...
                        connection.execute(findQuery, _query.values[0], {maxRows:10000, outFormat: oracledb.OBJECT}, function __FIND__(err, result) {
                            if (err) {
                                /* Release the connection back to the connection pool */
                                console.log("the encountered error is: " + err);
                                doRelease(connection);
                                return cb(handleQueryError(err, 'find'));
                            }
                            ...
                            // console.log("SAILS-ORA-ADAPTER: the typeOF of values is: " + typeof(values));
                            // console.log("SAILS-ORA-ADAPTER: out of find: " + JSON.stringify(values));
        ...
        create: function (connectionName, table, data, cb) {
            ...
            connectionObject.pool.getConnection(
                    function (err, connection)
                    {
                        ...
                        // var currentIdQuery ="select ID from " + table + " where rownum <=1 order by id desc";
                        var currentIdQuery ="select max(ID) as ID from " + table ;
                        async.waterfall([
                            function(callback) {
                        connection.execute(currentIdQuery, [], {maxRows:1, outFormat: oracledb.OBJECT}, function (err, res){
                            if(err){
                                // Release the connection back to the connection pool
                                console.log("OraADAPTER: following error occured with retrieving the latest id: " + err);
                                doRelease(connection);
                            }
                            // console.log("OraADAPTER: the resilt is: " + JSON.stringify(res));
                            if(!data.ID){
                            currentId=res.rows[0];
                            currentId=currentId.ID;
                            nextId=(parseInt(currentId,10)+1);
                            console.log("OraADAPTER: the currentId's value is: " + JSON.stringify(currentId) + " and the next id is: " + nextId);
                            finalQuery=finalQuery.replace(")", ', "ID")');
                            // console.log("the value of finalquery before replacing values" + finalQuery);

                            data.ID = nextId;
                            nextId=nextId+1;
                            valueString=Object.keys(data).map(function(k){return ("'" + data[k] + "'")}).join(",");
                            finalQuery=finalQuery.substring(0, finalQuery.indexOf('values'))
                            finalQuery=finalQuery + "values(" + valueString + ")";
                            var findNulls = new RegExp("'null'", "g");
                            finalQuery=finalQuery.replace(findNulls,"NULL");
                            callback();
                        }
                        else{
                            data.UPDATED_AT=moment(Date.parse(data.UPDATED_AT)).format('YYYY-MM-DD h:mm:ss');
                            data.CREATED_AT=moment(Date.parse(data.CREATED_AT)).format('YYYY-MM-DD h:mm:ss');
                            valueString=Object.keys(data).map(function(k){return ("'" + data[k] + "'")}).join(",");
                            finalQuery=finalQuery.substring(0, finalQuery.indexOf('values'))
                            finalQuery=finalQuery + "values(" + valueString + ")";
                            var findNulls = new RegExp("'null'", "g");
                            finalQuery=finalQuery.replace(findNulls,"NULL");
                            // console.log("the value of the the parsed data updated at is: " + dateField(data.UPDATED_AT));
                            console.log('OraADAPTER: The bind values post data modification is:: ' + JSON.stringify(data));
                            callback();
                        }

                            // console.log("OraADAPTER: the value of valueString is: " + valueString);
                            // console.log("OraADAPTER: the output of finalQuery is: " + finalQuery);
                             // Release the connection back to the connection pool
                             // doRelease(connection);
                    ...
                    // function(finalQuery, callback) {
                    function(callback) {
                        console.log('OraADAPTER: Executing query: ' + finalQuery);
                        // console.log('OraADAPTER: The bind values: ' + JSON.stringify(data));
                        // connection.execute(finalQuery, _.values(data), {/*outFormat: oracledb.OBJECT, */autoCommit: false}, function __CREATE__(err, result) {
                        connection.execute(finalQuery, [], {/*outFormat: oracledb.OBJECT, */autoCommit: true}, function __CREATE__(err, result) {
                        ...
                        callback();
                            ...
                            if (error) { console.log("the resulting error is: " + error); }
        ...
        update: function(connectionName, table, options, data, cb) {
            ...
            // data.UPDATED_AT=moment(data.UPDATED_AT).isValid() ? moment(data.UPDATED_AT).format('YYYY-MM-DD h:mm:ss') : "";
            // data.CREATED_AT=moment(data.CREATED_AT).isValid() ? moment(data.CREATED_AT).format('YYYY-MM-DD h:mm:ss') : "";
            ...
            connectionObject.pool.getConnection(
                    function (err, connection)
                    {
                        ...
                        if (LOG_QUERIES) {
                            ...
                            console.log('SAILS-ORA-ADAPTER: the following is the supplied data: ' + JSON.stringify(data));
                        }
                        connection.execute("alter session set NLS_DATE_FORMAT = 'YYYY-MM-DD HH24:MI:SS'", [], {maxRows:1, outFormat: oracledb.OBJECT}, function (err, res){
                             // Release the connection back to the connection pool
                                    doRelease(connection);
